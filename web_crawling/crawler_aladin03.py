...
# 브라우저 안뜨게 하기 (브라우저는 안뜨지만 크롤링은 정상 작동)
...
# 웹 페이지 전체가 로딩 될 때 까지 대기 후 남은 시간 무시
...
while True:
    ...
    for div_ss_book_box in div_ss_book_box_list:
        ...
        # 이미지 
        img_url = div_ss_book_box.select_one('table div > a img.front_cover') # 요소 하나만 지목하는 메서드
        # print(img_url)
        ...
        author_name = author_data[0].strip() # 양쪽 공백 제거
        company = author_data[1].strip() # 양쪽 공백 제거
        pub_day = author_data[2].strip() # 양쪽 공백 제거
    ...
    del soup # 현재페이지에서 사용했던 beautifulsoup 객체를 삭제
